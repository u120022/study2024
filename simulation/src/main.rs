...
impl LeftTurnedData {
    fn sample(rng: &mut impl rand::Rng, var: &LeftTurnedVar, distr: &LeftTurnedDistr) -> Self {
        ...
        for &[_, x] in &position_series {
            if x_0 <= x && x < x_1 {
                curvature_series.push([x, a1.powi(2).recip() * (x - x_0)]);
            } else if x_1 <= x && x < x_2 {
                curvature_series.push([x, r_min.recip()]);
            } else if x_2 <= x && x < x_3 {
                curvature_series.push([x, r_min.recip() - a2.powi(2).recip() * (x - x_2)]);
            } else {
                curvature_series.push([x, 0.0]);
            }
        }
